...
### 2.17 論文重新定位：便宜商用 WiFi + 演算法貢獻（`csi_synth/PAPER_POSITIONING.md`）

因應「實驗改以 ESP32/52 子載波為主、AX211/256 留作對照」的現實，把論文的主張/貢獻/範圍釘清楚
（PR #16）。核心定位：**子載波數是硬體參數、不是貢獻；貢獻是演算法、系統與實證發現，且它們在
受限硬體下更有價值。**

- **主打賣點（真實實驗掙得）**：「對商用 AP 的 CSI 感測，**取樣均勻性是一階決定因素**，常比子載波數、
  比標稱採樣率更重要」——以 §2.15 的均勻 6Hz 勝過爆發式 125Hz 為實證。翻轉「要昂貴硬體才行」的預設。
- **貢獻清單**：C0 取樣均勻性原則（放第一）、C1 數位孿生/sim-to-real、C2 PASS（子載波少時更關鍵）、
  C3 雙任務模型、C4 場域校準、C5 普惠系統——全部不以「用了更多子載波」為賣點。
- **52 vs 256**：把 256 從主角降為「scaling 特性軸」，用孿生模擬/公開 AX211 資料畫「子載波數 vs 效能」
  曲線，論證核心任務在 52 已足、256 買到的是難場景（多人/穿牆/定位）的餘裕。
- **範圍聲明**：已驗證主張限定**單人**呼吸/呼吸中止；多人/複雜動作明列為 future work（真正槓桿是
  天線數+頻寬+演算法，非僅子載波數）。附一段可直接改寫進 Abstract 的話。
...
AP，§2.16）→ #16 論文重新定位（便宜商用硬體+演算法貢獻，§2.17）。全數已合併 `main`。
