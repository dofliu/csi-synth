...
**B. 孿生 UI 端到端佐證** `tools/twin_ui_bench.mjs`(headless Chromium 驅動打包後的 HTML)：6 個代表情境的畫面偵測 **6/6 皆合理**(呼吸/淨空/走路/EMI失敗旗標/ESP32邊緣)。過程發現兩個真實行為：(1) 偵測需 **~10s 暖機**填滿 DFT 緩衝，讀太早會假次峰誤報；(2) 對很強單人呼吸，「訊號混疊」偵測會**瞬態邊緣誤報**——屬教學展示層可調門檻，嚴謹量化以 Python 為準。
